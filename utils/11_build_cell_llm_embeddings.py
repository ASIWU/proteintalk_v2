#!/usr/bin/env python3
"""Build frozen LLM text embeddings for training-ready Cell values."""
...
def collect_cell_records(training_ready_root: Path, dataset_group: str) -> list[dict[str, Any]]:
    meta_path = training_ready_root / dataset_group / "global_meta.json"
    meta = load_json(meta_path)
    mapping = meta["value_to_index"]["Cell"]
    ...
    for task_name in meta.get("task_names", []):
        ...
        if "Cell_index" in df.columns:
            index_values = pd.to_numeric(df["Cell_index"], errors="coerce").fillna(0).astype(int).tolist()
        for row_offset, raw_value in enumerate(df["Cell"].tolist()):
            if index_values is None:
                ...
                if index not in records:
                    raise ValueError(f"{task_dir}: Cell_index={index} is not present in global Cell mapping")
...
def description_prompt(record: dict[str, Any], dataset_group: str = "") -> str:
    ...
    if group == "ptv1":
        return (
            "Write one concise biomedical description for a breast-cancer cell-line label used in "
            "PTV1 drug-response proteomics experiments. Focus on known tissue lineage, tumor subtype "
            "or receptor context when established, common oncogenic or phenotypic traits, and why this "
            "cell line may matter for perturbation response. If the label is not a well-known cell line, "
            "avoid unsupported claims and describe it as an observed breast-cancer cell-line label. "
            "Use 2-4 sentences, no bullets.\n\n"
            "Dataset context: PTV1 AIVC and extra single-drug breast-cancer proteomics.\n"
            f"Cell-line label: {name}\n"
            f"Observed labels: {variants}"
        )
    return (
        "Write one concise biomedical description for a cancer cell-line or Cell label used in "
        "drug-response proteomics experiments. Focus on tissue lineage, tumor context, common "
        "biological traits, and why the label may matter for perturbation response. Use 2-4 sentences, "
        "no bullets. "
        f"Cell label: {name}. Observed labels: {variants}."
    )
...
def build_embeddings(args: argparse.Namespace) -> None:
    ...
    if output_path.exists() and sidecar_path.exists() and not args.force:
        print(f"[cell-llm] existing artifact kept: {output_path}")
        return

    records = collect_cell_records(Path(args.training_ready_root), args.dataset_group)
    ...
    for record in records:
        ...
        if canonical == "no":
            description = "Missing or unspecified cell label. Reserved zero-vector row."
        else:
        ...
        print(f"[cell-llm] described index={record['index']} name={canonical}")
    ...
    print(f"[cell-llm] wrote {output_path} shape={embedding_matrix.shape}")
    print(f"[cell-llm] wrote {sidecar_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default=str(DEFAULT_TRAINING_READY_ROOT / "ptv3" / "derived" / "cell_llm_embedding_qwen3_4096.npz"),
    )
