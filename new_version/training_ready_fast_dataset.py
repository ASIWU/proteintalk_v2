...
This module intentionally lives under ``new_version`` and does not modify the
existing dataset implementation.  It keeps the full protein expression axis but
loads expression matrices with numpy memmap so each worker reads only the rows
needed for a batch.
"""
...
class FastProteinTalkDataset(Dataset):
    ...
    def _format_pair(self, control_row: int, perturb_row: int) -> dict[str, np.ndarray]:
        ...
        output = {
            "control_expression": np.array(self.expression_matrix[control_row], dtype=np.float32, copy=True),
            "perturb_expression": np.array(self.expression_matrix[perturb_row], dtype=np.float32, copy=True),
            ...
            "covariates": self._covariates[perturb_row],
