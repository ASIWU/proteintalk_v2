...
class FastDeltaDrugResponseModel(nn.Module):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        pair_hidden = self._encode_drug_pair(batch["drug_embeddings"].float(), graph_features)
    ...
    def _encode_drug_pair(self, drug_embeddings: torch.Tensor, graph_features: torch.Tensor | None = None) -> torch.Tensor:
