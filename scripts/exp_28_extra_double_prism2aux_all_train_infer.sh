...
EXPERIMENT_SET_NAME="extra_double_prism2aux_all_train_infer"

# Match the validated PRISM2-aux double-drug 5-fold default used by exp_26.
...
ptv3_print_settings "Baseline4 PRISM2-aux train on all single+double data, then infer extra double-drug datasets"
ptv3_run_preflight

all_single_double_exp="${EXP_PREFIX}_all_single_double_prism2aux_for_extra"
...
if [[ -n "${REFERENCE_5FOLD_CKPT_PATH}" ]]; then
  ...
  reference_epoch="$(ptv3_reference_epoch \
    ...
    ptv3_main_doubledrug_prism2aux \
    synergy \
  ...
  echo "[checkpoint-policy] extra double PRISM2-aux uses reference ${REFERENCE_EPOCH_AGG} epoch=${reference_epoch}; training all-single-double for max_epochs=${reference_max_epochs} and using last.ckpt"
  train_args+=(--max-epochs "${reference_max_epochs}" --monitor none)
fi

ptv3_train "${all_single_double_exp}" \
  ptv3_main_doubledrug_prism2aux all_train_subset_test synergy \
  "${train_args[@]}"
