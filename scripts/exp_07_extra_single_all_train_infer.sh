...
if [[ -n "${REFERENCE_5FOLD_CKPT_PATH}" ]]; then
  ...
  echo "[checkpoint-policy] extra single uses reference ${REFERENCE_EPOCH_AGG} epoch=${reference_epoch}; training all-single for max_epochs=${reference_max_epochs} and using last.ckpt"
