...
EXPRESSION_LATENT_DIM="${EXPRESSION_LATENT_DIM:-768}"
COVARIATE_EMBEDDING_DIM="${COVARIATE_EMBEDDING_DIM:-96}"
...
MSE_TARGET_MODE="${MSE_TARGET_MODE:-all}"
MSE_TARGET_TOPK="${MSE_TARGET_TOPK:-512}"
MSE_TARGET_PPI_TOPK="${MSE_TARGET_PPI_TOPK:-32}"
MSE_TARGET_PPI_ALPHA="${MSE_TARGET_PPI_ALPHA:-0.5}"
MSE_TARGET_PPI_NORM="${MSE_TARGET_PPI_NORM:-raw}"
MSE_TARGET_DEGREE_PENALTY="${MSE_TARGET_DEGREE_PENALTY:-0.0}"
MSE_TARGET_CHUNK_SIZE="${MSE_TARGET_CHUNK_SIZE:-64}"
MSE_TARGET_CACHE_DIR="${MSE_TARGET_CACHE_DIR:-}"
FORCE_MSE_TARGET_CACHE_REBUILD="${FORCE_MSE_TARGET_CACHE_REBUILD:-0}"
MSE_TARGET_VARIANCE_TOPK="${MSE_TARGET_VARIANCE_TOPK:-4096}"
MSE_TARGET_VARIANCE_SCALE="${MSE_TARGET_VARIANCE_SCALE:-2.0}"
MSE_WEIGHT_SCHEDULE="${MSE_WEIGHT_SCHEDULE:-constant}"
MSE_DECAY_START_EPOCH_FRAC="${MSE_DECAY_START_EPOCH_FRAC:-0.4}"
MSE_FINAL_WEIGHT_MULTIPLIER="${MSE_FINAL_WEIGHT_MULTIPLIER:-1.0}"
MSE_PRETRAIN_EPOCHS="${MSE_PRETRAIN_EPOCHS:-0}"
MSE_PRETRAIN_BCE_WEIGHT="${MSE_PRETRAIN_BCE_WEIGHT:-0.0}"
...
RESPONSE_BASE_LOGIT_SCALE="${RESPONSE_BASE_LOGIT_SCALE:-1.0}"
RESPONSE_DELTA_MODE="${RESPONSE_DELTA_MODE:-off}"
RESPONSE_DELTA_DIM="${RESPONSE_DELTA_DIM:-64}"
RESPONSE_DELTA_SEED="${RESPONSE_DELTA_SEED:-31}"
RESPONSE_DELTA_DETACH="${RESPONSE_DELTA_DETACH:-0}"
DELTA_LOGIT_SCALE="${DELTA_LOGIT_SCALE:-0.0}"
DELTA_LOGIT_LEARNABLE="${DELTA_LOGIT_LEARNABLE:-0}"
...
RANKING_LOSS_HARD_NEGATIVES="${RANKING_LOSS_HARD_NEGATIVES:-0}"
RANKING_LOSS_HARD_POSITIVES="${RANKING_LOSS_HARD_POSITIVES:-0}"
DELTA_TEACHER_LOSS_WEIGHT="${DELTA_TEACHER_LOSS_WEIGHT:-0.0}"
...
CELL_LLM_MODE="${CELL_LLM_MODE:-off}"
...
CONTROL_DRUG_INTERACTION_MODE="${CONTROL_DRUG_INTERACTION_MODE:-off}"
CONTROL_DRUG_INTERACTION_SCALE="${CONTROL_DRUG_INTERACTION_SCALE:-0.0}"
CONTROL_DRUG_LOGIT_SCALE="${CONTROL_DRUG_LOGIT_SCALE:-0.0}"
OBSERVED_PERTURB_EXPRESSION_MODE="${OBSERVED_PERTURB_EXPRESSION_MODE:-off}"
OBSERVED_PERTURB_EXPRESSION_SCALE="${OBSERVED_PERTURB_EXPRESSION_SCALE:-0.0}"
OBSERVED_PERTURB_LOGIT_SCALE="${OBSERVED_PERTURB_LOGIT_SCALE:-0.0}"
...
USE_DOSE_COVARIATE="${USE_DOSE_COVARIATE:-0}"
DOSE_COVARIATE_FIELDS="${DOSE_COVARIATE_FIELDS:-pert_dose1 pert_dose2}"
USE_DDI="${USE_DDI:-0}"
ZERO_INIT_DELTA_HEAD="${ZERO_INIT_DELTA_HEAD:-0}"
CONTROL_EXPRESSION_DROPOUT="${CONTROL_EXPRESSION_DROPOUT:-0.0}"
...
COMMON_TRAIN_ARGS=(
  ...
  --mse-target-mode "${MSE_TARGET_MODE}"
  --mse-target-topk "${MSE_TARGET_TOPK}"
  --mse-target-ppi-topk "${MSE_TARGET_PPI_TOPK}"
  --mse-target-ppi-alpha "${MSE_TARGET_PPI_ALPHA}"
  --mse-target-ppi-norm "${MSE_TARGET_PPI_NORM}"
  --mse-target-degree-penalty "${MSE_TARGET_DEGREE_PENALTY}"
  --mse-target-chunk-size "${MSE_TARGET_CHUNK_SIZE}"
  --mse-target-variance-topk "${MSE_TARGET_VARIANCE_TOPK}"
  --mse-target-variance-scale "${MSE_TARGET_VARIANCE_SCALE}"
  --mse-weight-schedule "${MSE_WEIGHT_SCHEDULE}"
  --mse-decay-start-epoch-frac "${MSE_DECAY_START_EPOCH_FRAC}"
  --mse-final-weight-multiplier "${MSE_FINAL_WEIGHT_MULTIPLIER}"
  --mse-pretrain-epochs "${MSE_PRETRAIN_EPOCHS}"
  --mse-pretrain-bce-weight "${MSE_PRETRAIN_BCE_WEIGHT}"
  ...
  --target-expression-ppi-norm "${TARGET_EXPRESSION_PPI_NORM}"
  --target-expression-degree-penalty "${TARGET_EXPRESSION_DEGREE_PENALTY}"
  ...
  --target-expression-cell-gate-mode "${TARGET_EXPRESSION_CELL_GATE_MODE}"
  --target-expression-cell-gate-scale "${TARGET_EXPRESSION_CELL_GATE_SCALE}"
  --target-expression-cell-gate-temperature "${TARGET_EXPRESSION_CELL_GATE_TEMPERATURE}"
  ...
  --response-base-logit-scale "${RESPONSE_BASE_LOGIT_SCALE}"
  --response-delta-mode "${RESPONSE_DELTA_MODE}"
  --response-delta-dim "${RESPONSE_DELTA_DIM}"
  --response-delta-seed "${RESPONSE_DELTA_SEED}"
  --delta-logit-scale "${DELTA_LOGIT_SCALE}"
  ...
  --ranking-loss-hard-negatives "${RANKING_LOSS_HARD_NEGATIVES}"
  --ranking-loss-hard-positives "${RANKING_LOSS_HARD_POSITIVES}"
  --delta-teacher-loss-weight "${DELTA_TEACHER_LOSS_WEIGHT}"
  ...
  --cell-llm-mode "${CELL_LLM_MODE}"
  --cell-llm-fusion-mode "${CELL_LLM_FUSION_MODE}"
  --cell-llm-condition-scale "${CELL_LLM_CONDITION_SCALE}"
  --cell-llm-logit-scale "${CELL_LLM_LOGIT_SCALE}"
  --cell-llm-dropout "${CELL_LLM_DROPOUT}"
  --cell-type-llm-mode "${CELL_TYPE_LLM_MODE}"
  --cell-type-llm-fusion-mode "${CELL_TYPE_LLM_FUSION_MODE}"
  --cell-type-llm-condition-scale "${CELL_TYPE_LLM_CONDITION_SCALE}"
  --cell-type-llm-logit-scale "${CELL_TYPE_LLM_LOGIT_SCALE}"
  --cell-type-llm-dropout "${CELL_TYPE_LLM_DROPOUT}"
  --control-drug-interaction-mode "${CONTROL_DRUG_INTERACTION_MODE}"
  --control-drug-interaction-scale "${CONTROL_DRUG_INTERACTION_SCALE}"
  --control-drug-logit-scale "${CONTROL_DRUG_LOGIT_SCALE}"
  --observed-perturb-expression-mode "${OBSERVED_PERTURB_EXPRESSION_MODE}"
  --observed-perturb-expression-scale "${OBSERVED_PERTURB_EXPRESSION_SCALE}"
  --observed-perturb-logit-scale "${OBSERVED_PERTURB_LOGIT_SCALE}"
  --covariate-unk-dropout "${COVARIATE_UNK_DROPOUT}"
  --control-expression-dropout "${CONTROL_EXPRESSION_DROPOUT}"
  --seed "${SEED}"
)
...
if [[ -n "${EARLY_STOPPING_PATIENCE}" ]]; then
  COMMON_TRAIN_ARGS+=(--early-stopping-patience "${EARLY_STOPPING_PATIENCE}")
  COMMON_TRAIN_ARGS+=(--early-stopping-min-delta "${EARLY_STOPPING_MIN_DELTA}")
fi
...
if [[ -n "${MSE_TARGET_CACHE_DIR}" ]]; then
  COMMON_TRAIN_ARGS+=(--mse-target-cache-dir "${MSE_TARGET_CACHE_DIR}")
fi
if [[ "${FORCE_MSE_TARGET_CACHE_REBUILD}" == "1" ]]; then
  COMMON_TRAIN_ARGS+=(--force-mse-target-cache-rebuild)
fi
if [[ "${RESPONSE_DELTA_DETACH}" == "1" ]]; then
  COMMON_TRAIN_ARGS+=(--response-delta-detach)
fi
if [[ "${DELTA_LOGIT_LEARNABLE}" == "1" ]]; then
  COMMON_TRAIN_ARGS+=(--delta-logit-learnable)
fi
if [[ -n "${CELL_LLM_EMBEDDING_PATH}" ]]; then
  COMMON_TRAIN_ARGS+=(--cell-llm-embedding-path "${CELL_LLM_EMBEDDING_PATH}")
fi
if [[ -n "${CELL_TYPE_LLM_EMBEDDING_PATH}" ]]; then
  COMMON_TRAIN_ARGS+=(--cell-type-llm-embedding-path "${CELL_TYPE_LLM_EMBEDDING_PATH}")
fi
if [[ "${USE_DDI}" == "1" ]]; then
  COMMON_TRAIN_ARGS+=(--use-ddi)
fi
if [[ "${ZERO_INIT_DELTA_HEAD}" == "1" ]]; then
  COMMON_TRAIN_ARGS+=(--zero-init-delta-head)
fi
...
if [[ "${USE_DOSE_COVARIATE}" == "1" ]]; then
  read -r -a DOSE_COVARIATE_FIELD_LIST <<< "${DOSE_COVARIATE_FIELDS}"
  COMMON_TRAIN_ARGS+=(--use-dose-covariate --dose-covariate-fields "${DOSE_COVARIATE_FIELD_LIST[@]}")
fi
...
if [[ "${RESPONSE_DELTA_DETACH}" == "1" ]]; then
  COMMON_INFER_ARGS+=(--response-delta-detach)
fi
if [[ "${DELTA_LOGIT_LEARNABLE}" == "1" ]]; then
  COMMON_INFER_ARGS+=(--delta-logit-learnable)
fi
if [[ -n "${CELL_LLM_EMBEDDING_PATH}" ]]; then
  COMMON_INFER_ARGS+=(--cell-llm-embedding-path "${CELL_LLM_EMBEDDING_PATH}")
fi
if [[ -n "${CELL_TYPE_LLM_EMBEDDING_PATH}" ]]; then
  COMMON_INFER_ARGS+=(--cell-type-llm-embedding-path "${CELL_TYPE_LLM_EMBEDDING_PATH}")
fi
if [[ -n "${TARGET_EXPRESSION_CACHE_DIR}" ]]; then
...
if [[ "${ZERO_INIT_DELTA_HEAD}" == "1" ]]; then
  COMMON_INFER_ARGS+=(--zero-init-delta-head)
fi
if [[ -n "${BATCH_COV_LIST}" ]]; then
  if [[ "${BATCH_COV_LIST}" == "__none__" ]]; then
    COMMON_INFER_ARGS+=(--batch-cov-list)
  else
    read -r -a BATCH_COV_FIELD_LIST <<< "${BATCH_COV_LIST}"
    COMMON_INFER_ARGS+=(--batch-cov-list "${BATCH_COV_FIELD_LIST[@]}")
  fi
fi
if [[ "${USE_DOSE_COVARIATE}" == "1" ]]; then
  read -r -a DOSE_COVARIATE_FIELD_LIST <<< "${DOSE_COVARIATE_FIELDS}"
  COMMON_INFER_ARGS+=(--use-dose-covariate --dose-covariate-fields "${DOSE_COVARIATE_FIELD_LIST[@]}")
fi
ptv3_utc_now() {
...
ptv3_print_settings() {
  ...
  echo "[settings] GRAPH_FEATURE_MODE=${GRAPH_FEATURE_MODE}; GRAPH_STRUCTURAL_RP=${GRAPH_STRUCTURAL_RP}; GRAPH_DRUG_CONCAT=${GRAPH_DRUG_CONCAT}; GRAPH_PAIR_ADD_SCALE=${GRAPH_PAIR_ADD_SCALE}; GRAPH_LOGIT_SCALE=${GRAPH_LOGIT_SCALE}; USE_DDI=${USE_DDI}; ZERO_INIT_DELTA_HEAD=${ZERO_INIT_DELTA_HEAD}; CONTROL_EXPRESSION_DROPOUT=${CONTROL_EXPRESSION_DROPOUT}; PROTEIN_CONCAT_MODE=${PROTEIN_CONCAT_MODE}"
  ...
  echo "[settings] RESPONSE_BASE_LOGIT_SCALE=${RESPONSE_BASE_LOGIT_SCALE}; RESPONSE_DELTA_MODE=${RESPONSE_DELTA_MODE}; RESPONSE_DELTA_DIM=${RESPONSE_DELTA_DIM}; RESPONSE_DELTA_DETACH=${RESPONSE_DELTA_DETACH}; DELTA_LOGIT_SCALE=${DELTA_LOGIT_SCALE}; DELTA_LOGIT_LEARNABLE=${DELTA_LOGIT_LEARNABLE}"
  ...
  echo "[settings] CELL_PAIR_FILM_SCALE=${CELL_PAIR_FILM_SCALE}; TARGET_EXPRESSION_MODE=${TARGET_EXPRESSION_MODE}; TARGET_EXPRESSION_FUSION_MODE=${TARGET_EXPRESSION_FUSION_MODE}; TARGET_EXPRESSION_TOPK=${TARGET_EXPRESSION_TOPK}; TARGET_EXPRESSION_PPI_TOPK=${TARGET_EXPRESSION_PPI_TOPK}; TARGET_EXPRESSION_PPI_ALPHA=${TARGET_EXPRESSION_PPI_ALPHA}; TARGET_EXPRESSION_PPI_NORM=${TARGET_EXPRESSION_PPI_NORM}; TARGET_EXPRESSION_DEGREE_PENALTY=${TARGET_EXPRESSION_DEGREE_PENALTY}"
  echo "[settings] TARGET_EXPRESSION_CELL_GATE_MODE=${TARGET_EXPRESSION_CELL_GATE_MODE}; TARGET_EXPRESSION_CELL_GATE_SCALE=${TARGET_EXPRESSION_CELL_GATE_SCALE}; TARGET_EXPRESSION_CELL_GATE_TEMPERATURE=${TARGET_EXPRESSION_CELL_GATE_TEMPERATURE}"
  echo "[settings] FOCAL_LOSS=${FOCAL_LOSS}; FOCAL_GAMMA=${FOCAL_GAMMA}; FOCAL_ALPHA=${FOCAL_ALPHA}"
  echo "[settings] RANKING_LOSS_WEIGHT=${RANKING_LOSS_WEIGHT}; RANKING_LOSS_GROUP_FIELD=${RANKING_LOSS_GROUP_FIELD}; RANKING_LOSS_HARD_NEGATIVES=${RANKING_LOSS_HARD_NEGATIVES}; RANKING_LOSS_HARD_POSITIVES=${RANKING_LOSS_HARD_POSITIVES}; DELTA_TEACHER_LOSS_WEIGHT=${DELTA_TEACHER_LOSS_WEIGHT}; CELL_PRIOR_MODE=${CELL_PRIOR_MODE}; CELL_PRIOR_LOGIT_SCALE=${CELL_PRIOR_LOGIT_SCALE}; CELL_PRIOR_FIXED_LOGIT_SCALE=${CELL_PRIOR_FIXED_LOGIT_SCALE}"
  echo "[settings] CELL_LLM_MODE=${CELL_LLM_MODE}; CELL_LLM_FUSION_MODE=${CELL_LLM_FUSION_MODE}; CELL_LLM_CONDITION_SCALE=${CELL_LLM_CONDITION_SCALE}; CELL_LLM_LOGIT_SCALE=${CELL_LLM_LOGIT_SCALE}; CELL_LLM_DROPOUT=${CELL_LLM_DROPOUT}; CELL_LLM_EMBEDDING_PATH=${CELL_LLM_EMBEDDING_PATH:-default}"
  echo "[settings] CELL_TYPE_LLM_MODE=${CELL_TYPE_LLM_MODE}; CELL_TYPE_LLM_FUSION_MODE=${CELL_TYPE_LLM_FUSION_MODE}; CELL_TYPE_LLM_CONDITION_SCALE=${CELL_TYPE_LLM_CONDITION_SCALE}; CELL_TYPE_LLM_LOGIT_SCALE=${CELL_TYPE_LLM_LOGIT_SCALE}; CELL_TYPE_LLM_DROPOUT=${CELL_TYPE_LLM_DROPOUT}; CELL_TYPE_LLM_EMBEDDING_PATH=${CELL_TYPE_LLM_EMBEDDING_PATH:-default}"
  echo "[settings] CONTROL_DRUG_INTERACTION_MODE=${CONTROL_DRUG_INTERACTION_MODE}; CONTROL_DRUG_INTERACTION_SCALE=${CONTROL_DRUG_INTERACTION_SCALE}; CONTROL_DRUG_LOGIT_SCALE=${CONTROL_DRUG_LOGIT_SCALE}"
  echo "[settings] OBSERVED_PERTURB_EXPRESSION_MODE=${OBSERVED_PERTURB_EXPRESSION_MODE}; OBSERVED_PERTURB_EXPRESSION_SCALE=${OBSERVED_PERTURB_EXPRESSION_SCALE}; OBSERVED_PERTURB_LOGIT_SCALE=${OBSERVED_PERTURB_LOGIT_SCALE}"
  ...
  echo "[settings] MSE_TARGET_MODE=${MSE_TARGET_MODE}; MSE_TARGET_TOPK=${MSE_TARGET_TOPK}; MSE_TARGET_PPI_TOPK=${MSE_TARGET_PPI_TOPK}; MSE_TARGET_PPI_ALPHA=${MSE_TARGET_PPI_ALPHA}; MSE_TARGET_PPI_NORM=${MSE_TARGET_PPI_NORM}; MSE_TARGET_DEGREE_PENALTY=${MSE_TARGET_DEGREE_PENALTY}"
  echo "[settings] MSE_WEIGHT_SCHEDULE=${MSE_WEIGHT_SCHEDULE}; MSE_DECAY_START_EPOCH_FRAC=${MSE_DECAY_START_EPOCH_FRAC}; MSE_FINAL_WEIGHT_MULTIPLIER=${MSE_FINAL_WEIGHT_MULTIPLIER}; MSE_PRETRAIN_EPOCHS=${MSE_PRETRAIN_EPOCHS}; MSE_PRETRAIN_BCE_WEIGHT=${MSE_PRETRAIN_BCE_WEIGHT}"
  ...
  echo "[settings] BATCH_COV_LIST=${BATCH_COV_LIST:-train.py-default}; USE_DOSE_COVARIATE=${USE_DOSE_COVARIATE}; DOSE_COVARIATE_FIELDS=${DOSE_COVARIATE_FIELDS}"
  ...
  echo "[settings] SEED=${SEED}"
  echo "[settings] BEST_CKPT_METRIC=${BEST_CKPT_METRIC}; MONITOR=${MONITOR:-auto}; MONITOR_MODE=${MONITOR_MODE:-auto}; ALLOW_NONFINITE_MONITOR=${ALLOW_NONFINITE_MONITOR}; EARLY_STOPPING_PATIENCE=${EARLY_STOPPING_PATIENCE:-off}; EARLY_STOPPING_MIN_DELTA=${EARLY_STOPPING_MIN_DELTA}"
