...
OUTPUT_MONTH="${OUTPUT_DATE:0:7}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/${OUTPUT_MONTH}/${OUTPUT_DATE}}"
...
RESPONSE_TRAJECTORY_MODE="${RESPONSE_TRAJECTORY_MODE:-off}"
RESPONSE_TRAJECTORY_DIM="${RESPONSE_TRAJECTORY_DIM:-64}"
RESPONSE_TRAJECTORY_SEED="${RESPONSE_TRAJECTORY_SEED:-37}"
RESPONSE_TRAJECTORY_DETACH="${RESPONSE_TRAJECTORY_DETACH:-0}"
TRAJECTORY_LOGIT_SCALE="${TRAJECTORY_LOGIT_SCALE:-0.0}"
TRAJECTORY_LOGIT_LEARNABLE="${TRAJECTORY_LOGIT_LEARNABLE:-0}"
...
CONTROL_EXPRESSION_MODE="${CONTROL_EXPRESSION_MODE:-real}"
RANDOM_CONTROL_EXPRESSION_PATH="${RANDOM_CONTROL_EXPRESSION_PATH:-}"
...
COMMON_TRAIN_ARGS=(
  ...
  --response-trajectory-mode "${RESPONSE_TRAJECTORY_MODE}"
  --response-trajectory-dim "${RESPONSE_TRAJECTORY_DIM}"
  --response-trajectory-seed "${RESPONSE_TRAJECTORY_SEED}"
  --trajectory-logit-scale "${TRAJECTORY_LOGIT_SCALE}"
  ...
  --control-expression-mode "${CONTROL_EXPRESSION_MODE}"
  --random-control-expression-path "${RANDOM_CONTROL_EXPRESSION_PATH}"
  --seed "${SEED}"
)
...
if [[ "${RESPONSE_TRAJECTORY_DETACH}" == "1" ]]; then
  COMMON_TRAIN_ARGS+=(--response-trajectory-detach)
fi
if [[ "${TRAJECTORY_LOGIT_LEARNABLE}" == "1" ]]; then
  COMMON_TRAIN_ARGS+=(--trajectory-logit-learnable)
fi
...
if [[ -n "${DRUG_EMBEDDING_PATH}" ]]; then
  COMMON_TRAIN_ARGS+=(--drug-embedding-path "${DRUG_EMBEDDING_PATH}")
fi
if [[ "${USE_DDI}" == "1" ]]; then
...
if [[ "${RESPONSE_TRAJECTORY_DETACH}" == "1" ]]; then
  COMMON_INFER_ARGS+=(--response-trajectory-detach)
fi
if [[ "${TRAJECTORY_LOGIT_LEARNABLE}" == "1" ]]; then
  COMMON_INFER_ARGS+=(--trajectory-logit-learnable)
fi
if [[ -n "${CELL_LLM_EMBEDDING_PATH}" ]]; then
...
if [[ -n "${DRUG_EMBEDDING_PATH}" ]]; then
  COMMON_INFER_ARGS+=(--drug-embedding-path "${DRUG_EMBEDDING_PATH}")
fi
if [[ -n "${TARGET_EXPRESSION_CACHE_DIR}" ]]; then
...
ptv3_print_settings() {
  ...
  echo "[settings] RESPONSE_TRAJECTORY_MODE=${RESPONSE_TRAJECTORY_MODE}; RESPONSE_TRAJECTORY_DIM=${RESPONSE_TRAJECTORY_DIM}; RESPONSE_TRAJECTORY_DETACH=${RESPONSE_TRAJECTORY_DETACH}; TRAJECTORY_LOGIT_SCALE=${TRAJECTORY_LOGIT_SCALE}; TRAJECTORY_LOGIT_LEARNABLE=${TRAJECTORY_LOGIT_LEARNABLE}"
  ...
  echo "[settings] CONTROL_EXPRESSION_MODE=${CONTROL_EXPRESSION_MODE}; RANDOM_CONTROL_EXPRESSION_PATH=${RANDOM_CONTROL_EXPRESSION_PATH:-none}"
  echo "[settings] DRUG_EMBEDDING_PATH=${DRUG_EMBEDDING_PATH:-default}"
  ...
  echo "[settings] OUTPUT_DATE=${OUTPUT_DATE}; OUTPUT_DIR=${OUTPUT_DIR}"
  echo "[settings] TRAINING_READY_ROOT=${TRAINING_READY_ROOT}"
}
...
ptv3_run_preflight() {
  ...
  "${PYTHON_BIN}" -m py_compile \
    ...
    scripts/generate_random_control_proteome.py \
  ...
  if [[ "${RUN_DATA_VALIDATION}" == "1" ]]; then
    ...
    "${PYTHON_BIN}" utils/03_validate_training_ready_outputs.py --output-root "${TRAINING_READY_ROOT}"
  fi
